...
export class OPRFBaseMode implements OPRFFn {
    ...
    async evaluate(key: Uint8Array, blinded: Uint8Array): Promise<Uint8Array> {
        ...
        const deserBlinded = Elt.deserialize(server.gg, blinded)
    ...
    async finalize(
        ...
        const deserEval = Elt.deserialize(client.gg, evaluationBytes)
        const blindSc = Scalar.deserialize(client.gg, blind)
...
export function tripleDH_IKM(cfg: Config, keys: scalarElt3): Uint8Array {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        const point = Elt.deserialize(gg, pk)
        const scalar = Scalar.deserialize(gg, sk)
