...
import { OPRFClient, Oprf, Scalar, SuiteID } from '@cloudflare/voprf-ts'

import { jest } from '@jest/globals'
import vectors from './testdata/vectors_v09.json'

async function createMocks(vector: (typeof vectors)[number], cfg: Config, isFake: boolean) {
    jest.clearAllMocks()
    ...
    if (!isFake) {
        ...
        jest.spyOn(OPRFClient.prototype, 'randomBlinder').mockImplementationOnce(() => {
            ...
            const scalar = Scalar.deserialize(group, blind)
...
async function test_full_registration(
    input: inputTest,
...
async function test_full_login(
    input: inputTest,
    vector: (typeof vectors)[number]
): Promise<boolean> {
    // Setup
